...
void adsb_ext_message_parse(uint8_t full_message[ADSB_EXT_LEN],
	...
	char
		parity_str[3 * 3]; // Each byte: "XX " (3 chars), 3 bytes -> 9 chars

	snprintf(data_str, sizeof(data_str),
			"%02X %02X %02X %02X %02X %02X %02X", msg_ext.data[0],
			msg_ext.data[1], msg_ext.data[2], msg_ext.data[3],
			msg_ext.data[4], msg_ext.data[5], msg_ext.data[6]);

	snprintf(parity_str, sizeof(parity_str), "%02X %02X %02X",
			msg_ext.parity[0], msg_ext.parity[1], msg_ext.parity[2]);
